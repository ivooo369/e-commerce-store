...
import { Office, OfficeMapProps } from "@/lib/interfaces";
  ...
  const centerMapOnLocation = useCallback(
    (lat: number, lng: number, zoom: number = 12) => {
      ...
      if (
        ...
        console.error("Невалидни координати:", { lat, lng });
      }
    },
    []
  );
  ...
  const cleanupMarkers = useCallback(() => {
    ...
    markersRef.current.forEach((marker) => {
      try {
        ...
        console.error("Възникна грешка при премахване на маркера:", e);
      }
    });
    markersRef.current = [];
  }, []);
    ...
    try {
      mapRef.current.eachLayer((layer) => {
        try {
          ...
          console.error("Възникна грешка при премахване на слоя:", e);
        }
      });
      ...
      console.error("Възникна грешка при изчистване на картата:", e);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    if (!mapRef.current && mapContainerRef.current) {
      try {
        ...
        console.error("Възникна грешка при инициализиране на картата:", e);
        return;
      }
    }
    ...
    const updateMap = () => {
      ...
      try {
        ...
        const validOffices = offices.filter((office) => {
          ...
          if (!hasValidCoords) {
            console.warn("Пропускане на офис поради невалидни координати:", {
              id: office.id,
              name: office.name,
        ...
        Object.entries(officeGroups).forEach(
          ([coords, officesAtLocation], index) => {
            ...
            `;
              ...
              console.error(
                "Възникна грешка при добавяне на маркер за местоположение:",
                coords,
                e
              );
            }
        ...
        console.error("Възникна грешка при обновяване на картата:", error);
      }
    };
