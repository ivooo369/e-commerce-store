...
export interface UseFavoritesReturn {
  ...
  isToggling: (productCode: string | undefined) => boolean;
}

export interface OrderNotesModalProps {
  open: boolean;
  onClose: () => void;
  notes?: string | null;
}
...
export interface CheckoutStepsProps {
  ...
  formErrors: { agreeTerms?: string };
  offices: Office[];
  isLoadingOffices: boolean;
  officesError: Error | null;
  onInputChange: (
    e: React.ChangeEvent<
      HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement
    >
  ) => void;
  onDeliveryMethodChange: (method: "speedy" | "econt" | "address") => void;
  onFormDataChange: (
    updater: (prev: FormDataDelivery) => FormDataDelivery
  ) => void;
  onSettlementSelect: (settlement: Settlement) => void;
}
